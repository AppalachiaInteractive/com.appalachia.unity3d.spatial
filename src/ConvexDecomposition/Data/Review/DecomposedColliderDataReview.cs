...
namespace Appalachia.Spatial.ConvexDecomposition.Data.Review
{
    ...
    public class DecomposedColliderDataReview : SingletonAppalachiaObject<DecomposedColliderDataReview>
    {
        ...
        private static readonly ProfilerMarker _PRF_RebuildList = new ProfilerMarker(_PRF_PFX + nameof(RebuildList));
        private static readonly ProfilerMarker _PRF_SortPiecesAsc = new ProfilerMarker(_PRF_PFX + nameof(SortPiecesAsc));
        private static readonly ProfilerMarker _PRF_SortVerticesAsc = new ProfilerMarker(_PRF_PFX + nameof(SortVerticesAsc));
        private static readonly ProfilerMarker _PRF_SortTrianglesAsc = new ProfilerMarker(_PRF_PFX + nameof(SortTrianglesAsc));
        private static readonly ProfilerMarker _PRF_SortPiecesDesc = new ProfilerMarker(_PRF_PFX + nameof(SortPiecesDesc));
        private static readonly ProfilerMarker _PRF_SortVerticesDesc = new ProfilerMarker(_PRF_PFX + nameof(SortVerticesDesc));
        private static readonly ProfilerMarker _PRF_SortTrianglesDesc = new ProfilerMarker(_PRF_PFX + nameof(SortTrianglesDesc));
        ...
        [ListDrawerSettings(NumberOfItemsPerPage = 4, HideAddButton = true, HideRemoveButton = true, DraggableItems = false)]
        ...
        public void ProcessAllSuggested()
        {
            ...
            using (var progress = new EditorOnlyProgressBar("Updating to external model...", eligible, true, 1))
            {
                for (var i = 0; i < index.Count; i++)
                {
                    if (progress.Cancellable && progress.Cancelled)
            ...
            using (_PRF_RebuildList.Auto())
            {
                ...
                for (var i = 0; i < dataList.Count; i++)
                {
                    ...
                    var item = new DecomposedColliderDataReviewItem
                    {
                        ...
                        vertices = new DecomposedColliderDataReviewItemData() {original = data.originalMesh.vertexCount},
                        triangles = new DecomposedColliderDataReviewItemData() {original = data.originalMesh.triangles.Length/3},
