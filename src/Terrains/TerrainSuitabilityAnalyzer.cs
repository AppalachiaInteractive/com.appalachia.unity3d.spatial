...
namespace Appalachia.Spatial.Terrains
{
    public class TerrainSuitabilityAnalyzer : InternalMonoBehaviour
    {
        ...
        [BoxGroup("HUD"), PropertyRange(0.01f, 2.0f)]
        public float hudSize = 1.0f;
        
        [BoxGroup("HUD"), PropertyRange(0.001f, 0.1f)]
        public float hudSphereSize = 0.1f;
        
        [BoxGroup("HUD")] public Vector3 hudOffset = new Vector3(0.2f, 0.2f, 0.5f);
        
        [BoxGroup("HUD")] public Vector3 labelOffset = new Vector3(0.1f, 0.1f, 0.0f);
        ...
        [BoxGroup("Setup")]
        public SuitabilityType suitabilityType;
        
        [BoxGroup("Setup")]
        public LocationSamplingMode SamplingMode;
        
        [BoxGroup("Setup"), PropertyRange(1, 4)]
        public int SampleResolution;
        
        [BoxGroup("Setup"), PropertyRange(1, 4)]
        public int SampleLevels;

        [BoxGroup("Setup"), PropertyRange(1.0f, 30.0f)]
        public float MinDistance;

        private float minMaxDistance => MinDistance + 1;
        
        [BoxGroup("Setup"), PropertyRange(nameof(minMaxDistance), 100.0f)]
        public float MaxDistance;
        
        [BoxGroup("Setup"), PropertyRange(0.1f, 1.0f)]
        ...
        [BoxGroup("Show Concavity/Concavity"), PropertyRange(0.01f, 2.0f)]
        ...
        [BoxGroup("Show Depression/Depression"), PropertyRange(0.01f, 20.0f)]
        ...
        [BoxGroup("Show Aspect/Aspect"), PropertyRange(0.01f, 2.0f)]
        public float AspectTolerance;
        [BoxGroup("Show Aspect/Aspect")]
        public bool AspectInverse;

        private float2 up => new float2(0.0f, 1.0f);
        private float2 right => new float2(1.0f, 0.0f);
        private float2 down => new float2(0.0f, -1.0f);
        private float2 left => new float2(-1.0f, 0.0f);
        ...
        string GetDirectionalString(float direction)
        {
        ...
        public string AspectReferenceDirectionXZ_Label => $"{GetDirectionalString(AspectReferenceDirection)}";
        ...
        public void OnDrawGizmosSelected()
        {
            ...
            float3 rootPosition_TS = rootPosition_WS - terrainPosition_WS;
            
            var rootPosition_XZ_TS_N = new float2(rootPosition_TS.x / terrainData.size.x, rootPosition_TS.z / terrainData.size.z);
            
            var rootHeight = terrainData.GetInterpolatedHeight(rootPosition_XZ_TS_N.x, rootPosition_XZ_TS_N.y);

            var rootNormal = terrainData.GetInterpolatedNormal(rootPosition_XZ_TS_N.x, rootPosition_XZ_TS_N.y);
            ...
            for (var ringIndex = 0; ringIndex < SampleLevels; ringIndex++)
            {
                ...
                var radius = SampleLevels == 1 ? MinDistance : math.lerp(MinDistance, MaxDistance, ringTime);
                ...
                for (var pointIndex = 0; pointIndex < samplePointCount; pointIndex++)
                {
                    ...
                    var samplePosition_N = new float2(samplePositionX_TS / terrainData.size.x, samplePositionZ_TS / terrainData.size.z);
                    
                    var sampleHeight = terrainData.GetInterpolatedHeight(samplePosition_N.x, samplePosition_N.y);
                    
                    var sampleNormal = math.normalizesafe(terrainData.GetInterpolatedNormal(
                        samplePosition_N.x,
                        samplePosition_N.y
                    ));
                    ...
                    var samplePosition_TS = new float3(samplePositionX_TS, sampleHeight, samplePositionZ_TS);
                    ...
                    var sampleValue = suitabilityType == SuitabilityType.Aspect
                        ...
                        : suitabilityType == SuitabilityType.Concavity
                            ? GetConcavitySampleValue(samplePosition_TS, rootPosition_TS, sampleNormal)
        ...
        private float4 InitializeAspectSample(
            float2 centerNormal,
            float2 referenceDirection)
        {
        ...
        private float GetConcavitySampleValue(float3 samplePosition, float3 rootPosition, float3 sampleNormal)
        {
        ...
        private float GetTestValue(float sampleMin, float sampleMax, float sampleSum, int sampleCount)
        {
            float testValue;
